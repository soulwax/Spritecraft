...
void main() {
  ...
  group('ExportSupport.writeEnginePresetFiles', () {
    test('writes both engine presets when requested', () async {
      ...
      final Map<String, dynamic> unityPayload = jsonDecode(
        await files.firstWhere((File file) => file.path.endsWith('.unity.json'))
            .readAsString(),
      ) as Map<String, dynamic>;
    ...
    test('writes aseprite and generic companion files when requested', () async {
      final Directory root = await Directory.systemTemp.createTemp(
      ...
      final List<File> asepriteFiles = await ExportSupport.writeEnginePresetFiles(
        exportDirectory: root,
      ...
      expect(path.basename(asepriteFiles.single.path), 'hero-export.aseprite.json');
      final Map<String, dynamic> asepritePayload = jsonDecode(
        await asepriteFiles.single.readAsString(),
      ) as Map<String, dynamic>;
      ...
      expect(asepritePayload['meta']['exportOptions']['cropMode'], 'trim-transparent');

      final List<File> genericFiles = await ExportSupport.writeEnginePresetFiles(
        exportDirectory: root,
        baseName: 'hero-export',
      ...
      expect(path.basename(genericFiles.single.path), 'hero-export.generic.json');
      final Map<String, dynamic> genericPayload = jsonDecode(
        await genericFiles.single.readAsString(),
      ) as Map<String, dynamic>;
  ...
  group('ExportSupport.writeCreditsArtifacts', () {
    test('writes shipping-friendly credits and license companion files', () async {
      final Directory root = await Directory.systemTemp.createTemp(
      ...
      final Map<String, dynamic> creditsJson = jsonDecode(
        await files
            .firstWhere((File file) => file.path.endsWith('.credits.json'))
            .readAsString(),
      ) as Map<String, dynamic>;
      expect(creditsJson['format'], 'spritecraft.credits');
      expect(creditsJson['licenses'], containsAll(<String>['CC-BY-SA 3.0', 'OGA-BY 3.0']));
